...
class CreateCategoryScreenDesktop extends StatelessWidget {
   CreateCategoryScreenDesktop({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    // 💡 Reset categories when screen opens
               ...
               TRoundedContainer(
                ...
                borderColor: dark ? Colors.grey.withOpacity(0.5) : TColors.dark.withOpacity(0.2),
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        TTableHeader(buttonText: 'Create New Category', onPressed: () => Get.toNamed(TRoutes.createCategory),searchController: _searchController,
                        searchOnChanged: (query) => categoryController.filterCategory(query),),
                        SizedBox(height: TSizes.spaceBetwwenItems,),
                    
                         RepaintBoundary(child: CategoryTable())
                      ],
                    ),
                  ),
                ),
