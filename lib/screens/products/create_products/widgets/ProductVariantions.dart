...
class ProductVariantions extends StatefulWidget {
  const ProductVariantions({super.key});

  @override
  State<ProductVariantions> createState() => _ProductVariantionsState();
}

class _ProductVariantionsState extends State<ProductVariantions> {
...
Future<void> _loadFee() async {
  ...
  Widget _builVariationTitle(
   ...
   Obx(() {
  double productPrice = variationController.variationPriceMap[variation] ?? 0.0;

  double chargeToReceive = PaymentCalculator.calculateAmountToCharge(productPrice, double.tryParse(feePro.toString())!);
  double netAfterFee = PaymentCalculator.calculateNetAmountAfterFee(productPrice, double.tryParse(feePro.toString())!);
  double feeAmount = PaymentCalculator.calculateFee(productPrice, double.tryParse(feePro.toString())!);

  return Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      if (variation.price == null) Text('For Discount Price'),

      if (processingFeeValue.toString() == 'include')
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('If you want to RECEIVE ${productPrice.toStringAsFixed(2)} LKR:'),
      ...
      if (processingFeeValue.toString() != 'include')
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('If you CHARGE ${productPrice.toStringAsFixed(2)} LKR:'),
            Text('Net after fee: ${netAfterFee.toStringAsFixed(2)} LKR'),
